    ...
    internal class IsEquivalentListMatcher<T> : NonNullDiagnosingMatcher<IEnumerable<T>>
    {
        ...
        protected override bool MatchesSafely(IEnumerable<T> collection, IDescription mismatchDescription)
        {
            ...
            for (var i = 0; i < Math.Max(collectionArray.Length, matcherArray.Length); i++)
            {
                ...
                {
                    mismatchDescription.AppendFormat("was too short (expected to be of length {0}, was {1})", matcherArray.Length, collectionArray.Length);
                    return false;
                }

                if (i >= matcherArray.Length)
                {
                    mismatchDescription.AppendFormat("was too long (expected to be of length {0}, was {1})", matcherArray.Length, collectionArray.Length);
                    return false;
                }

                if (!matcherArray[i].Matches(collectionArray[i]))
                {
                    mismatchDescription.AppendFormat("was not matched at position {0}:", i);
